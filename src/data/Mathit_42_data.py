...
def evaluate_and_wrap(eqs: List[Equation], cfg, DIM, return_n=False):
    ...
    if DIM == 1:
        support = sample_support_dim1(eqs[0], curr_p, cfg)
    elif DIM == 2:
        support = sample_support_dim2(eqs[0], curr_p, cfg)
    elif DIM == 3:
        support = sample_support_dim3(eqs[0], curr_p, cfg)
    elif DIM == 4:
        support = sample_support_dim4(eqs[0], curr_p, cfg)
    elif DIM == 5:
        support = sample_support_dim5(eqs[0], curr_p, cfg)
    else:
        raise NotImplementedError
    ...
    assert DIM == 1
    x, ys = res[0,:,0], res[:,:,DIM]

    batch_size = 32

    _ys = []
    for ii in range(batch_size):
        idxs = np.random.choice(len(ys), 41)
        y = ys[idxs].unsqueeze(0)
        _ys.append(y)
    _ys = torch.cat(_ys, axis=0)

    batch = torch.cat([_ys, torch.ones_like(_ys)], axis=1)
    batch = torch.cat([batch, x.unsqueeze(0).unsqueeze(0).expand(batch_size, 1, batch.size(2))], axis=1)
...
class NeSymReS42DataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_root,
        train_path,
        ...
        self.nesymres_data_cfg = nesymres_data_cfg
        ...
        if stage == "fit" or stage is None:
            ...
            if self.data_train_path:
                self.training_dataset = NesymresDataset(
                    ...
                    self.nesymres_data_cfg.dataset_train,
                    mode="train"
                )
            ...
            if self.data_val_path:
                self.validation_dataset = NesymresDataset(
                    self.data_val_path,
                    self.nesymres_data_cfg.dataset_val,
                    mode="val"
                )
            
            if self.data_test_path:
                self.test_dataset = NesymresDataset(
                    self.data_test_path, self.nesymres_data_cfg.dataset_test,
                    mode="test"
                )
    ...
    def train_dataloader(self):
        ...
        trainloader = torch.utils.data.DataLoader(
            ...
            shuffle=True,
            drop_last=True,
            collate_fn=partial(custom_collate_fn,cfg= self.nesymres_data_cfg.dataset_train, DIM=len(self.training_dataset.cfg.total_variables)),
    ...
    def val_dataloader(self):
        ...
        validloader = torch.utils.data.DataLoader(
            ...
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=partial(custom_collate_fn,cfg= self.nesymres_data_cfg.dataset_val, DIM=len(self.validation_dataset.cfg.total_variables)),
            num_workers=self.num_workers,
            pin_memory=True,
            drop_last=False
        )
    ...
    def test_dataloader(self):
        ...
        testloader = torch.utils.data.DataLoader(
            ...
            batch_size=self.batch_size,
            shuffle=False,
            collate_fn=partial(custom_collate_fn,cfg=self.nesymres_data_cfg.dataset_test, DIM=len(self.test_dataset.cfg.total_variables)),
            num_workers=self.num_workers,
            pin_memory=True,
            drop_last=False
        )
